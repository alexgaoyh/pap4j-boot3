...
/**
 * <p>全路径树形结构扁平化提取算子。</p>
 * <p>该算子会递归遍历树结构，将每个节点的完整路径都加入结果列表（包含中间节点和叶子节点）。</p>
 * <p>例如：A -> B -> C，结果将包含 [A, A > B, A > B > C]</p>
 * <p>用法：TREE_FLATTEN(root, 'childrenKey', 'valueKey', 'separator')</p>
 */
public class TreeFlattenOperator implements CustomFunction {

    @Override
    public Object call(QContext qContext, Parameters parameters) throws Throwable {
        if (parameters.size() < 3) {
            throw new IllegalArgumentException("TREE_FLATTEN 需要至少3个参数：根对象, 子节点Key, 属性Key, [路径分隔符]");
        }
