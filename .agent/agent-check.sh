...
if command -v mvn &> /dev/null; then
    ...
    mvn checkstyle:check "-Dmaven.gitcommitid.skip=true" "-Dfile.encoding=UTF-8" "$@"
else
    echo "[AI-Agent-Runner] System 'mvn' not found. Falling back to project wrapper 'mvnw'..."
    "$SCRIPT_DIR/../mvnw" checkstyle:check "-Dmaven.gitcommitid.skip=true" "-Dfile.encoding=UTF-8" "$@"
fi
