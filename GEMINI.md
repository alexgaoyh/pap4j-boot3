...
*   **保证编译通过**: 每次完成代码修改后，如果涉及公共模块，应建议或主动运行相关的单元测试（如 `mvn clean test -pl <模块名> -Dtest=<测试类> "-Dfile.encoding=UTF-8" "-Dmaven.gitcommitid.skip=true" "-DskipTests=false"`）以确保没有破坏现有逻辑。
