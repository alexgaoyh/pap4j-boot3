...
### 7.4 数据更新与置空规范（NULL 语义强约束）
*   **严禁盲目依赖持久层默认置空**：凡涉及“解绑关联、清空字段”需落库为 `NULL` 的场景，严禁假设持久层或 ORM 会自动处理。
*   **原因**：
    *   绝大多数持久层框架和代码生成器默认采用“非空检查（Selective）”策略，会自动忽略入参中的 `null`，导致置空操作在 DB 层静默失效。
    *   若绕过持久层实体机制强写原生更新，极易导致全局审计字段（如修改时间、修改人）静默丢失。
*   **实现要求**：
    *   ✅ **模型层显式声明**：经常涉及置空/解绑的业务字段，必须在持久化实体模型上显式声明支持覆盖/全量更新策略。
    *   ✅ **审计保全**：无论采用何种覆写方式，必须确保修改时间、修改人等全局审计机制正常触发，严禁丢失更新痕迹。
    *   ❌ **禁止项**：严禁直接使用“忽略 null / 选择性更新”方法去处理数据解绑与清空业务。
*   **测试验收**：凡含解绑/清空诉求的接口，测试用例必须覆盖“从有值置为 NULL”的反向场景，严格核验数据库落库结果。
...
13. **防御性安全 (接口安全)**: 免去用户鉴权的公开/回调接口是否引入了防篡改、防重放或可信凭证校验等安全防护机制？
14. **防御性安全 (跨域过滤)**: 跨域策略是否使用精准域名白名单，避免了反射任意 Origin 或在允许凭证时使用 `*` 通配符？
...
17. **持久层置空/解绑 (NULL 语义)**: 涉及字段清空或解除关联时，是否确保底层更新不会因非空过滤（Selective 机制）而静默忽略？全局审计跟踪（如更新时间、更新人）是否依然正常触发？测试用例是否覆盖了从有值置为 NULL 的反向落库核验？
