...
1. 使用 Git 状态命令查看当前暂存与工作区状态（Git Bash / PowerShell 通用）：
   ```bash
   git status
   ```
...
2. 运行未尽 Task 对应的验证命令（Git Bash / PowerShell 通用），快速重设上下文与断点。

---

## ⚡ 命令行与验证规范 (Git Bash / PowerShell Compliance)
- Git 命令（`git status`、`git diff` 等）在 Git Bash 和 PowerShell 中通用。
  ```bash
  # 查看当前未提交代码改动
  git diff
  ```
