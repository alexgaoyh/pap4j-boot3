...
2. 使用搜索命令进行关键词模糊与精确搜索（按环境选择）：
   ```bash
   # Git Bash 环境（如 Claude Code）
   grep -rn "<TargetMethodName>" pap4j-common/ --include="*.java"
   ```
   ```powershell
   # PowerShell 环境
...
### 步骤 3：单测验证 (Verification)
使用 `./.agent/agent-test.cmd` 验证新增工具单测（Git Bash / PowerShell 通用）：
```bash
...
- **Git Bash 环境（如 Claude Code）**：使用 `grep`、`find` 等命令搜索。
- **PowerShell 环境**：使用 `Select-String`、`Get-ChildItem` 等原生命令。
- 测试命令参数必须包裹双引号 `"-Dtest=..."`（两环境通用）。
