...
* **主导角色**: 架构师
* **目标**: 制定精准的修改计划。
* **规划**: 触发 `AI.md` 强阻断（#1-#6：破坏性契约变更、算法级优化、≥3 文件联动、依赖与配置变更、持久层红线、`pap4j-common` 公共 API 修改）时必须开启设计规划模式 **`[Plan]`**；其余改动走 `[QuickPlan]` 轻量确认。
...
* **行动 (Act)**: 委托 **[task-executor 技能](./skills/task-executor/SKILL.md)** 编排执行：任务画像 → 自检清单 → 前置门控 → 逐项 `[Edit]` → 逐项 `[Shell]` 验证 → 打勾。期间 [整洁代码实践者] 执行代码修改与写入操作 **`[Edit]`**；[代码守卫] 强制进行修改前审计。
